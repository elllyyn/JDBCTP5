...
public class MaterielDAO extends DAO{
	...
	public static Materiel materielexist(Materiel materiel, Magasin magasin) {
		try {			
			String sql = "SELECT m.NomMat, mat.NomCat, NomCom FROM ContenuMagasin m INNER JOIN ContenuMateriel c INNER JOIN Materiel mat ON(m.nomMat = c.nomMat AND mat.NomMateriel = m.nomMat AND m.NomMag= ? ) WHERE m.nomMat = ?;";
